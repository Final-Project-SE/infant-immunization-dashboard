...
const Dashboard = () => {
  const {
    vaccines,
    isPending: isFetchingVaccines,
    error: vaccinesError,
  } = useGetVaccines();
  const {
    hs,
    isPending: isFetchingHealthStations,
    error: healthStationsError,
  } = useGetHealthStations();
  const {
    children,
    isPending: isFetchingChildren,
    error: childrenError,
  } = useGetChildren();
  ...
  const vaccinesPerCategory =
    vaccines?.reduce((acc: { [key: string]: number }, vaccine) => {
      acc[vaccine.category] = (acc[vaccine.category] || 0) + 1;
      return acc;
    }, {}) || {};

  const transformedVaccines = Object.entries(vaccinesPerCategory).map(
    ([category, count]) => ({ name: category, count: count || 0 })
  );
  ...
  const immunizationStatus =
    children?.reduce((acc: { [key: string]: number }, child: any) => {
      const key = child.isVaccineCompleted ? "Completed" : "Not Completed";
      acc[key] = (acc[key] || 0) + 1;
      return acc;
    }, {}) || {};

  const transformedImmunizationStatus = Object.entries(immunizationStatus).map(
    ([status, count]) => ({ name: status, count: count || 0 })
  );
